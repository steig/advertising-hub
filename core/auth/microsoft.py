"""Microsoft advertising platform authentication. See platforms/microsoft/ for details."""
from dataclasses import dataclass
from .base import BaseAuth, AuthToken

@dataclass
class MicrosoftAuth(BaseAuth):
    platform: str = "microsoft"
    def authenticate(self) -> AuthToken:
        raise NotImplementedError("See platforms/microsoft/patterns/ for auth setup")
    def refresh(self, token: AuthToken) -> AuthToken:
        raise NotImplementedError
    def validate(self, token: AuthToken) -> bool:
        raise NotImplementedError
